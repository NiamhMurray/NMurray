...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 13 16:13:03 2020
...
def plot_series(timarr, y, ylabel, mytitle):
    ...
    '''
    plt.figure()
    plt.scatter(timarr,y,label=ylabel)
    plt.xlabel("Years")
    plt.ylabel(ylabel)
    plt.title(mytitle)
    plt.legend()
...
# =============================================================================
    ...
    #Set up Loop to Read in Data and Extract Series 
    ystart = 1999
    yend = 2010
    ynow = ystart
    windlong = np.zeros(0)
    ...
    for y in range (ystart,yend):
        #deal with december first 
        dstart = datetime.date(ynow, 12,1)
        dend = datetime.date(ynow, 12, 31)
        dayarr, timearr, windarr = extract_series(fpath, fstem, lonpick, latpick, dstart, dend)
        windlong = np.append(windlong, windarr)
        daylong = np.append(daylong, dayarr)
        #Deal with January and Febuary 
        ynow = ynow+1
        dstart = datetime.date(ynow, 1,1)
        dend = datetime.date(ynow, 2, 28)
        dayarr, timearr, windarr = extract_series(fpath, fstem, lonpick, latpick, dstart, dend)
        windlong = np.append(windlong, windarr)
        daylong = np.append(daylong, dayarr)
    ...
    #plot_series(daylong, windlong, 'wind Speed  (m/s)', 'Wind Speed Time Series \n for London 1979-2018 from S2S Forecast')
    #Use Plot Histogram to calculate v98 and show histogram 
    median, upper_quartile, v98 = plot_histogram(windlong, 30, "paleturquoise", "Frequency of Windspeed \n in London (DJF) 1999-2010 from S2S Forecast \n with leadtime of 5 days","Wind Speed (m/s)", "Frequency")
    ssi_S2S_london = ssi_index(windlong, v98)
    acum_ssi_S2S_london = sum(ssi_S2S_london)
