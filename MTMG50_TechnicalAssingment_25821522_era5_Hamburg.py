...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 13 17:57:29 2020
...
def read_era(filename, fyear, iprint):
    ...
    # Convert to timedelta format and then add to startcal to make a list of 
    #dates.
...
def subset_field(alon, alat, lonpick, latpick):
    ...
    # The indices(intlon, intlat)correspond to the centre of the closest gridbox 
...
def plot_series(timarr, y, ylabel, c, mytitle):
    ...
    '''
    plt.figure()
    plt.plot(timarr,y,label=ylabel, c= c)
    plt.ylim(0, 20)
    plt.xlabel("Years")
    plt.ylabel(ylabel)
    plt.title(mytitle)
...
# =============================================================================
    ...
    #Hamburg using ERA5 Data
    plot_series(years, windarr, 'Wind Speed  (m/s)', "dimgray" ,\
                'ERA5 Wind Speed Time Series \n for Hamburg 1979-2018')
    
    #Using the plot histogrma function plot a histogram of wind speeds in Hamburg
    #also returns the median, upper quartile and 98th percentile 
    median, upper_quartile, v98, v50,v75 = plot_histogram(windarr, 35, "peachpuff","Historical Frequency of Windspeed \n in Hamburg 1979-2018 using ERA5 Data","Wind Speed (m/s)", "Frequency")  
    
    #compute the SSI index for Hamburg for each time step in the entire data set
    ssi_index_hamburg = np.array(ssi_index(windarr, v98))    
    
    #compute the total SSI index for hamburg over the dataset
    acum_index_hamburg = sum(ssi_index_hamburg)
